...
from Utils.constants import getOtherPlatforms
...
async def ping(req: WatchHistory, authorization: str = Header(None)):
    ...
    res = UserHistoryService.updateUserHistory(
        user_id, [req.movie_id, req.duration,req.day,req.hour])
    return res
...
async def ping(query: str,request:Request, authorization: str = Header(None)) -> []:
    ...
    try:
        ...
        # req=copy.deepcopy(parsed_dict)
        # for key in parsed_dict.keys():
        #     if (parsed_dict[key]=="unknown" or parsed_dict[key]=='') or (isinstance(parsed_dict[key],list) and (len(parsed_dict[key])==0 or parsed_dict[key][0]=='unknown')):
        #         del req[key]
        ...
        if len(req.keys())==2:
            ...
            proper = filterQueryWithFeatures(user_id,queryFeat,req)
            req["subscribed_platforms"] = getOtherPlatforms(req["subscribed_platforms"])
            other = filterQueryWithFeatures(user_id,queryFeat,req)
            return proper,other

        print(req)
        proper = filterQuery(user_id, req)
        req["subscribed_platforms"] = getOtherPlatforms(req["subscribed_platforms"])
        other = filterQuery(user_id,req)
        return proper,other
